    ...
    const fetchPolicies = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          const itPolicies = data.filter((p: PolicyItem) => p.category === 'employee' && p.subCategory === 'IT');
      ...
      <main className="pt-30 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="mb-12 text-center lg:text-left">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            IT <span className="text-[#009edb]">Policies</span>
          </h1>
          <p className={`text-lg max-w-2xl ${isLight ? 'text-gray-600' : 'text-gray-300'}`}>
            Information Technology policies and security guidelines for our employees.
          </p>
        </div>
        ...
        {textPolicies.length === 0 && pdfPolicies.length === 0 && (
          <div className="text-center py-20">
            <p className={`text-lg ${isLight ? 'text-gray-600' : 'text-gray-300'}`}>No IT policies available at the moment.</p>
          </div>
        )}
      </main>
