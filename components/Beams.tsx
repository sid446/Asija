...
const Beams: React.FC<BeamsProps> = ({
  ...
  const mouseTargetRef = useRef<Vec2>(new Vec2(0.5, 0.5));
  const mouseSmoothRef = useRef<Vec2>(new Vec2(0.5, 0.5));
  const pausedRef = useRef(paused);
  const gradTexRef = useRef<Texture | null>(null);
  const hoverDampRef = useRef(hoverDampness);
  const isVisibleRef = useRef(true);
  const meshRef = useRef<Mesh | null>(null);
  const triRef = useRef<Geometry | null>(null);
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      sm.x += ( tgt.x - sm.x ) * alpha;
      sm.y += ( tgt.y - sm.y ) * alpha;

      program.uniforms.uMouse.value = [sm.x, sm.y];
    ...
    return () => {
      ...
      try { (mesh as any).remove?.(); } catch {}
      try { program.remove(); } catch {}
      try { const tex = gradTexRef.current?.texture; if (tex && gl) gl.deleteTexture(tex); } catch {}
